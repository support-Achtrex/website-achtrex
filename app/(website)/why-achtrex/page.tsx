...
export default function WhyAchtrexPage() {
  ...
  return (
    <main className="min-h-screen bg-[#f4f4f4] text-slate-900">
      <InnerPageHeader title="Why Achtrex" subtitle="The Structural Intelligence Advantage" theme="purple" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {advantages.map((adv, idx) => (
              <motion.div 
                ...
                <span className="text-[#00a9ce] font-black text-[11px] uppercase tracking-widest mb-3 block opacity-80">Core Advantage 0{idx + 1}</span>
                <h3 className="text-2xl font-black mb-4 text-white tracking-tight">{adv.title}</h3>
                <p className="text-slate-200 leading-relaxed text-[15px] font-normal">{adv.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
