        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {members.length > 0 ? (
                members.map((member, index) => (
                    ...
                    >
                        <div className="relative h-[420px] w-full rounded-2xl overflow-hidden mb-6 border border-white/10 bg-transparent/5 backdrop-blur-sm group-hover:border-primary/50 transition-all duration-500 shadow-lg group-hover:shadow-primary/10">
                            {/* Gradient Overlay */}
                            ...
                            <div className="absolute inset-x-0 bottom-0 p-6 z-20 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out bg-black/60 backdrop-blur-md border-t border-white/10">
                                <div className="flex justify-center gap-4">
                                    <a href={member.linkedin || '#'} className="p-2 bg-transparent/10 rounded-full hover:bg-primary hover:text-white transition-colors text-white">
                                        <Linkedin className="w-5 h-5" />
                                    </a>
                                    <a href={member.twitter || '#'} className="p-2 bg-transparent/10 rounded-full hover:bg-primary hover:text-white transition-colors text-white">
                                        <Twitter className="w-5 h-5" />
                                    </a>
                                    <a href={`mailto:${member.email}`} className="p-2 bg-transparent/10 rounded-full hover:bg-primary hover:text-white transition-colors text-white">
